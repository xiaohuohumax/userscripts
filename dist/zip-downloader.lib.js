  ...
  class Crc32 {
    ...
    append(data) {
      ...
      for (let offset = 0, length = data.length | 0; offset < length; offset++) {
        crc = crc >>> 8 ^ table$1[(crc ^ data[offset]) & 255];
      }
      this.crc = crc;
    }
    get() {
  ...
  function getHeaderInfo(options) {
    ...
    if (zip64) {
      ...
      if (zip64ExtraFieldComplete) {
        const rawExtraFieldZip64View2 = getDataView(rawExtraFieldZip64);
        ...
        if (zip64DiskNumberStart) {
          setUint32(rawExtraFieldZip64View2, rawExtraFieldZip64Offset, diskNumberStart);
    ...
    if (extendedTimestamp) {
      ...
      let offset2 = 5;
      setUint32(extraFieldExtendedTimestampView, offset2, Math.floor(lastModDate.getTime() / 1e3));
      offset2 += 4;
      if (lastAccessDate) {
        setUint32(extraFieldExtendedTimestampView, offset2, Math.floor(lastAccessDate.getTime() / 1e3));
        offset2 += 4;
      }
      if (creationDate) {
        setUint32(extraFieldExtendedTimestampView, offset2, Math.floor(creationDate.getTime() / 1e3));
      }
      try {
  ...
  async function closeFile(zipWriter, comment, options) {
    ...
    for (const [indexFileEntry, fileEntry] of Array.from(files.values()).entries()) {
      ...
      if (externalFileAttributes) {
        setUint32(directoryView, directoryOffset2, externalFileAttributes);
      }
      directoryOffset2 += 4;
      setUint32(directoryView, directoryOffset2, zip642 && zip64Offset ? MAX_32_BITS : fileEntryOffset);
      directoryOffset2 += 4;
  ...
  configureWorker({
    initModule: (config2) => {
      if (!modulePromise) {
        ...
        modulePromise = initModule(wasmURI, config2);
      }
